...
// ContactFormData and ContactResponse are imported from @shared/types
// to avoid DRY violation (previously duplicated in repository interface)
...
export class ContactService {
  ...
  validateContactForm(data: Partial<ContactFormPayload>): {
    isValid: boolean;
    ...
    if (
      !data.name ||
      data.name.trim().length < VALIDATION_CONFIG.NAME.MIN_LENGTH
    ) {
    ...
    if (
      !data.message ||
      data.message.trim().length < VALIDATION_CONFIG.MESSAGE.MIN_LENGTH
    ) {
